...
| `npm run media -- --manifest <m> --check` | DRIFT DETECTION: re-shoot to temp + pixel-diff against committed screenshots; exits 1 past `--drift-threshold` (default 3%). Automated by `.github/workflows/media-drift.yml` (nightly; ships disabled — set the `MEDIA_DRIFT_RUNNER` repo variable to a runner that can reach the app) |
...
| `npm run docgen:plan -- --spec <openapi.json>` | Incremental docs plan: ADD/UPDATE/REVIEW/REMOVE/SKIP (see `docgen/README.md`). Add `--adopt` to register hand-authored pages the manifest does not track yet |
| `npm run drop-sample -- --yes` | Graduation step — delete the fictional projects sample `init-product --fresh` leaves behind, prune the PDF/docgen/KB manifests, and list every navbar item and prose link still pointing at it (`--dry-run` to preview) |
